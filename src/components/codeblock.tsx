    ...
    >
      {title ? (
        <div className="flex text-fd-muted-foreground items-center gap-2 h-9.5 border-b px-4">
          {typeof icon === 'string' ? (
            <div
              className="[&_svg]:size-3.5 text-(--section-color)"
              dangerouslySetInnerHTML={{
                __html: icon,
              }}
            />
          ) : (
            <span className="text-(--section-color)">{icon}</span>
          )}
          <figcaption className="flex-1 truncate text-(--section-color)">{title}</figcaption>
          {Actions({
            className: '-me-2',
